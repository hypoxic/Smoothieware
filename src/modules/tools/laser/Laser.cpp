...
#ifdef CNC
    THEKERNEL->slow_ticker->attach(std::min(2500UL, 1000000 / period), this, &Laser::set_proportional_power);
    //uint32_t tmp;
    //tmp = (period >= 2500UL) ? 2500UL : period; 
    //THEKERNEL->slow_ticker->attach(std::min(4000UL, (period <= 2500UL) ? 2500UL : period), this, &Laser::set_proportional_power);
    //THEKERNEL->slow_ticker->attach(std::min(4000UL, tmp), this, &Laser::set_proportional_power);
#else
